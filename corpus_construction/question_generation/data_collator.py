...
class T2TDataCollator():
    ...
    def __call__(self, batch: List) -> Dict[str, torch.Tensor]:
        ...
        """
        # print(batch[0].keys())
        # print([example['source_ids'] for example in batch])
        input_ids = torch.stack([torch.LongTensor(example['source_ids']) for example in batch])
        target_ids = torch.stack([torch.LongTensor(example['target_ids']) for example in batch])
